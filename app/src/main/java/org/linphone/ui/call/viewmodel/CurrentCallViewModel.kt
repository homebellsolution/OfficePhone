            ...
            if (LinphoneUtils.isCallOutgoing(call.state)) {
                ...
                if (call.state == Call.State.Connected) {
                   ...
                   /* viewModelScope.launch {
                        delay(300) // 300ms delay works well in most devices
                        withContext(Dispatchers.Main) {
                            if (ActivityCompat.checkSelfPermission(coreContext.context, Manifest.permission.RECORD_AUDIO)
                                == PackageManager.PERMISSION_GRANTED
                            ) {
                                if (call.conference != null) {
                                    call.conference?.microphoneMuted = false
                                } else {
                                    call.microphoneMuted = false
                                }
                                isMicrophoneMuted.postValue(false)
                                Log.i("$TAG Microphone auto-unmuted after delay")
                            } else {
